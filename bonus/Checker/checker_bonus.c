...
void	free_all(t_list **_a, t_list **_b, t_bool flag, op_t **table, char **line)
{
	safe_free(line);
...
int main(int ac, char **av)
{
	...
	if (FALSE == insert_operations(hash_table))
		free_all(&_a, &_b, TRUE, hash_table, NULL);
    line = get_next_line(0);
	while ((line))
	{
		if (FALSE == acces(hash_table, line, &_a, &_b))
			free_all(&_a, &_b, TRUE,hash_table, &line);
		safe_free(&line);
		line = get_next_line(0);
	}
	safe_free(&line);
	if (_b != NULL || !is_sorted(_a))
		...
		ft_putstr_fd("OK\n", 1);
	free_all(&_a, &_b, FALSE, hash_table, &line);
	return (0);
}
